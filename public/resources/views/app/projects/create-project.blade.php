...
@section('page_title', trans('g.create_project') . ' - ' . config('system.name'))
        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">{{ trans('g.create_project') }}</h3>
              <div class="card-options">
                {{ trans('g.create_project_info') }}
              </div>
            </div>
...
function getUserAvatars(ids) {
  ...
  for (var i in assignee) {
    ...
    ret += '<img src="' + user.avatar + '" data-placement="top" class="avatar m-1" data-title="' + 
    ...
    ret += '" data-toggle="popover" style=\'padding: 1px; border: 2px solid ' + user.role_color + '\'>';
  }
  return ret;
}
</script>
