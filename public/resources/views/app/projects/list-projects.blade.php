...
@section('page_title', trans('g.projects') . ' - ' . config('system.name'))
        ...
        <div class="col-12">
          ...
          <div class="card shadow-lg">
            <div class="card-header">
              <h3 class="card-title">{{ trans('g.projects') }}</h3>
            </div>
...
<script>
  var statuses = [];
<?php
$projects = \Platform\Models\Project::select('project_status_id');
...
if (! auth()->user()->can('all-projects')) {
  ...
  // Project managers
  $projects = $projects->orWhereHas('managers', function($query) {
    $query->where('users.id', auth()->user()->id);
  });
  // Users with task
  $projects = $projects->orWhereHas('tasks', function($query) {
...
$projects = $projects->groupBy('project_status_id')->get();
  
foreach ($projects as $project) {
?>
  statuses[<?php echo $project->status->id ?>] = {name: "<?php echo str_replace('"', '&quot;', $project->status->name) . ' (' . $project->where('project_status_id', $project->status->id)->count() . ')' ?>", color: '{{ $project->status->color }}'};
<?php
}
?>
  var datatable;
...
$(function() {
  ...
  datatable = $('#datatable').DataTable({
    ...
    stateSaveCallback: function(settings,data) {
      localStorage.setItem('projects_{{ auth()->user()->id }}_' + settings.sInstance, JSON.stringify(data));
    },
    stateLoadCallback: function(settings) {
      return JSON.parse(localStorage.getItem( 'projects_{{ auth()->user()->id }}_' + settings.sInstance ));
    },
    buttons: [
      {
        extend: 'collection',
        ...
        buttons: [
          {
            text: "Excel (.xlsx)",
            action: function ( e, dt, node, config ) { document.location = '{{ url('projects/export/xlsx') }}'; }
          }, {
            text: "Excel (.xls)",
            action: function ( e, dt, node, config ) { document.location = '{{ url('projects/export/xls') }}'; }
          }, {
            text: "Comma-separated (.csv)",
            action: function ( e, dt, node, config ) { document.location = '{{ url('projects/export/csv') }}'; }
          }<?php /*, {
            text: "HTML (.html)",
            action: function ( e, dt, node, config ) { document.location = '{{ url('projects/export/html') }}'; }
          }*/ ?>
        ]
      },
<?php if (auth()->user()->can('delete-project')) { ?>
      {
...
<?php } ?>
<?php if (auth()->user()->can('create-project')) { ?>
      {
        text: "<i class=\"material-icons\" style=\"position:relative;top:1px\">add</i><span class=\"d-none d-md-inline\"> {{ trans('g.project') }}</span>",
        attr: {
            title : '{{ trans('g.create_project') }}',
            'data-toggle' : 'tooltip'
        },
        className: 'btn btn-secondary rounded-0',
        action: function ( e, dt, node, config ) {
          document.location = '{{ url('projects/create') }}';
        }
      }
<?php } ?>
    ],
    ...
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          ret += '<div><span class="badge badge-secondary" style="background-color:' + row.status_color + '">' + row.status_name + '</span></div>';

          if (row.category != null) {
            ret += '<div><small>' + row.category + '</small></div>';
          }
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (parseInt(row.total_tasks) > 0) { 
            ret += '<div class="mx-auto mt-1 chart-circle" data-value="' + row.task_progress + '" data-thickness="3" data-color="blue" style="width:56px;height:56px; top:-2px">';
            ret += '<div class="chart-circle-value small text-center">' + row.completed_tasks + '/' + row.total_tasks + '</div>';
            ret += '</div>';
          } else {
            ret += '<div class="text-muted small text-center">{{ trans('g.no_tasks') }}</div>';
          }
...
<?php if (auth()->user()->can('edit-project')) { ?>
          ret += '  <a href="<?php echo url('projects/edit/') ?>/' + data + '" class="dropdown-item"><i class="dropdown-icon fe fe-edit-2"></i> {{ trans('g.edit') }}</a>';
<?php } ?>
<?php if (auth()->user()->can('view-project')) { ?>
          ret += '  <a href="<?php echo url('projects/view/') ?>/' + data + '" class="dropdown-item"><i class="dropdown-icon fe fe-search"></i> {{ trans('g.view') }}</a>';
<?php } ?>
<?php if (auth()->user()->can('delete-project')) { ?>
...
?>
    ...
    initComplete: function() {
      ...
      datatable.rows().every(function (index, value) {
        status[this.data().project_status_id] = this.data().status_name;
      });
      ...
      this.api().columns(1).every(function() {
          ...
          for (var i in statuses) {
            ...
            select.append( '<option value="' + i + '"' + selected + ' data-data=\'{"color": "' + statuses[i].color + '"}\'">' + statuses[i].name + '</option>' )
          }
/*
          column.data().unique().sort().each(function ( d, j ) {
            select.append( '<option value="' + d + '">' + status[d] + '</option>' )
          });*/
      });
...
function deleteRecords(ids) {
  Swal({
    ...
    if (result.value) {
      var jqxhr = $.ajax({
        url: "{{ url('projects/delete-projects') }}",
