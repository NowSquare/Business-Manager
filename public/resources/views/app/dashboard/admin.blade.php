              ...
              <div class="col-sm-6 col-lg-3">
                <div class="card p-3">
                  <div class="d-flex align-items-center">
                    ...
                    <div>
                      <h4 class="m-0"><a href="{{ url('users') }}">{{ $people->where('active', 1)->count() }} <small>{{ trans_choice('g.person_people', $people->where('active', 1)->count()) }}</small></a></h4>
                      <small class="text-muted">{{ $people->where('active', 0)->count() }} {{ trans('g.inactive') }}</small>
                    </div>
                  </div>
                </div>
              </div>
              <div class="col-sm-6 col-lg-3">
                <div class="card p-3">
                  <div class="d-flex align-items-center">
                    ...
                    <div>
                      <h4 class="m-0"><a href="{{ url('companies') }}">{{ $companies->where('active', 1)->count() }} <small>{{ trans_choice('g.company_companies', $companies->where('active', 1)->count()) }}</small></a></h4>
                      <small class="text-muted">{{ $companies->where('active', 0)->count() }} {{ trans('g.inactive') }}</small>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div class="col-lg-6">
                <div class="card">
                  <div class="card-header">
                    <h3 class="card-title">{{ trans('g.activity') }}</h3>
                  </div>
                  <div id="chart-activity" style="height: 15rem"></div>
                </div>
                <script>
                  	$(document).ready(function(){
                  		var chart = c3.generate({
                  			bindto: '#chart-activity',
                        transition: {
                          duration: 0
                        },
                        data: {
                            ...
                            xFormat: '%Y',
                            columns: [
                                ['x', <?php foreach ($date_range as $date => $parts) { echo "new Date('" . $date . "'),"; } ?>],
                                ['data1', <?php foreach ($date_range as $date => $parts) { echo (isset($notifications_by_date[$date])) ? $notifications_by_date[$date] . ',' : '0,'; } ?>],
                                ['data2', <?php foreach ($date_range as $date => $parts) { echo (isset($users_by_date[$date])) ? $users_by_date[$date] . ',' : '0,'; } ?>],
                                ['data3', <?php foreach ($date_range as $date => $parts) { echo (isset($companies_by_date[$date])) ? $companies_by_date[$date] . ',' : '0,'; } ?>],
                                ['data4', <?php foreach ($date_range as $date => $parts) { echo (isset($projects_by_date[$date])) ? $projects_by_date[$date] . ',' : '0,'; } ?>]
                            ],
                            ...
                            colors: {
                              'data1': tabler.colors["blue-lighter"],
                              'data2': tabler.colors["blue"],
                              'data3': tabler.colors["green"],
                              'data4': tabler.colors["red"]
                            },
                            names: {
                              'data1': '{{ trans('g.notifications') }}',
                              'data2': '{{ trans('g.people') }}',
                              'data3': '{{ trans('g.companies') }}',
                              'data4': '{{ trans('g.projects') }}'
                            }
                        },
                        axis: {
                            y: {
                ...
                <div class="row">
                  <div class="col-sm-6">
                    <div class="card">
                      ...
                      <div class="card-body">
                        <div id="chart-donut-people" style="height: 12rem;"></div>
                      </div>
                    </div>
...
<?php foreach ($roles as $name => $role) { ?>
                      					["<?php echo str_replace('"', '&quot;', $name); ?>", <?php echo $role['count'] ?>],
<?php } ?>
                      				],
                      				type: 'donut',
                      				colors: {
<?php foreach ($roles as $name => $role) { ?>
                      					"<?php echo str_replace('"', '&quot;', $name); ?>": "<?php echo $role['color'] ?>",
<?php } ?>
                      				},
                      				names: {
<?php foreach ($roles as $name => $role) { ?>
                      					"<?php echo str_replace('"', '&quot;', $name); ?>": "<?php echo str_replace('"', '&quot;', __($name)); ?>",
<?php } ?>
                      				}
                      			},
                      			axis: {
...
if ($projects->count() == 0) {
?>
                        <div class="text-muted lead" style="height: 12rem;">
                          {{ trans('g.nothing_to_show') }}
                        </div>
<?php
} else {
?>
                        <div id="chart-donut-project-statuses" style="height: 12rem;"></div>
<?php
}
?>
                      </div>
                    ...
                    <script>
                      	$(document).ready(function(){
                      		var chart = c3.generate({
                      			bindto: '#chart-donut-project-statuses',
                            transition: {
                              duration: 0
                            },
                      			data: {
                      				columns: [
<?php foreach ($project_statuses as $id => $status) { ?>
                      					["data<?php echo $id; ?>", <?php echo $status['count'] ?>],
<?php } ?>
                      				],
                      				type: 'donut',
                      				colors: {
<?php foreach ($project_statuses as $id => $status) { ?>
                      					"data<?php echo $id; ?>": "<?php echo $status['color'] ?>",
<?php } ?>
                      				},
                      				names: {
<?php foreach ($project_statuses as $id => $status) { ?>
                      					"data<?php echo $id; ?>": "<?php echo str_replace('"', '&quot;', __($status['name'])); ?>",
<?php } ?>
                      			  }
                      			},
                      			axis: {
...
foreach ($team_members as $user) {
  $sl = \Platform\Controllers\Core\Secure::array2string(['user_id' => $user->id]);
?>
                      <li class="list-separated-item">
                        <div class="row align-items-center">
                          ...
                          <div class="col">
                            <div>
                              <a href="{{ url('users/view/' . $sl) }}" class="text-inherit">{{ $user->active_role_name }}</a>
                            </div>
