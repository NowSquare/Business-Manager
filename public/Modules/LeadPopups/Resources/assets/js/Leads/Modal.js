...
let leadModal = {
  ...
  settings: {
    ...
    content: null, // Default content for modal
    template: 'default',
    submit_button: null, // Default submit button
    submit_button_class: '.btn .btn-success', // Default submit button class
    form_fields: null, // Default form fields
    after_submit_message: null,
    ...
    appKey: 'base64:ahvecFmvHiXzIKQAlPfnhXKlHaqt2Y/wVdJZ354vMmk=', // Laravel key (not the same as in the .env, any key is fine, it's used to prevent visitors from reading and editing cookies)
  },
  ...
  init: function(token, host, callback) {
    ...
    if (! document.getElementById(cssId)) {
      ...
      link.rel  = 'stylesheet';
      link.type = 'text/css';
      link.href = leadModal.settings.host + '/modules/popups/assets/style.css';
      link.media = 'all';
  ...
  loadSettings: function(callback) {
    if (leadModal.settings.token !== null) {
      ...
      // Load settings
      leadModal.getJSON(leadModal.settings.host + '/popups/settings?token=' + this.settings.token + '&lang=' + userLang.slice(0,2) + '&host=' + encodeURIComponent(window.location.host) + '&path=' + encodeURIComponent(window.location.pathname), function(err, data) {
        if (err !== null) {
  ...
  start: function() {
    ...
    function triggerModal() {
      ...
      setTimeout(function() {
        ...
        if (leadModal.settings.token === null && leadModal.settings.content !== null) {
          leadModal.settings.modalUrl = leadModal.settings.host + 
            '/popups/get/' + leadModal.settings.template + '?content=' + encodeURIComponent(leadModal.settings.content) + 
            '&submit_button=' + encodeURIComponent(leadModal.settings.submit_button) + 
            '&form_fields=' + encodeURIComponent(leadModal.settings.form_fields) + 
            '&after_submit_message=' + encodeURIComponent(leadModal.settings.after_submit_message) + 
            '&preview=1' + 
            '&locale=' + leadModal.settings.locale;
        } else {
          leadModal.settings.modalUrl = leadModal.settings.host + 
            '/popups/get/' + leadModal.settings.template + '/' + encodeURIComponent(leadModal.settings.token) + '/?locale=' + leadModal.settings.locale;
        }
        ...
        if (leadModal.settings.token === null) {
          ...
          xhr.onload = function() {
            ...
            if (status === 200 || status === 302) {
              ...
              frame.setAttribute('data', leadModal.settings.host + '/popups/get?locale=' + leadModal.settings.locale);
              content.appendChild(frame);
            }
          };
