...
@section('page_title', trans('coupons::g.coupons') . ' - ' . config('system.name'))
...
<?php } ?>
          ...
          <div class="card shadow-lg">
            <div class="card-header">
              <h3 class="card-title">{{ trans('coupons::g.coupons') }}</h3>
            </div>
              ...
              <table class="table table-striped table-hover table-borderless" id="datatable">
                <thead class="thead-dark">
                  <tr>
                    ...
                    <th>{{ trans('g.name') }}</th>
                    <th class="text-center">{{ trans('coupons::g.views') }}</th>
                    <th class="text-center">{{ trans('coupons::g.conversions') }}</th>
                    <th></th>
                  </tr>
                </thead>
                <tbody>
...
$(function() {
  ...
  datatable = $('#datatable').DataTable({
    ajax: "{{ url('coupons/json') }}",
    stateSaveCallback: function(settings,data) {
      localStorage.setItem('coupons_{{ auth()->user()->id }}_' + settings.sInstance, JSON.stringify(data));
    },
    stateLoadCallback: function(settings) {
      return JSON.parse(localStorage.getItem( 'coupons_{{ auth()->user()->id }}_' + settings.sInstance ));
    },
    buttons: [
      {
      ...
      {
        text: "<i class=\"material-icons\" style=\"position:relative;top:1px\">add</i><span class=\"d-none d-md-inline\"> {{ trans('coupons::g.coupon') }}</span>",
        attr: {
        ...
        action: function ( e, dt, node, config ) {
          document.location = '{{ url('coupons/create') }}';
        }
      }
    ],
    columns: [
    ...
    columnDefs: [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          ret += '<div><a href="' + row.url + '" class="text-muted small" target="_blank">';
          ret += row.url;
          ret += '</a></div>';

          if (row.active != 1) {
            ret += '<div class="badge badge-secondary">{!! trans('g.inactive') !!}</div>';
          }
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          ret += '  <a href="<?php echo url('coupons/edit/') ?>/' + data + '" class="dropdown-item"><i class="dropdown-icon fe fe-edit-2"></i> {{ trans('g.edit') }}</a>';
          ret += '  <a href="javascript:getQr(\'' + row.name.replace(/"/g, '&quot;').replace(/'/g, '\\\'') + '\', \'' + row.url + '\', \'' + row.qr + '\')" class="dropdown-item"><i class="dropdown-icon fas fa-qrcode"></i> {{ trans('coupons::g.view_qr') }}</a>';
...
function deleteRecords(ids) {
  Swal({
    ...
    if (result.value) {
      var jqxhr = $.ajax({
        url: "{{ url('coupons/delete-coupons') }}",
...
function getQr(title, url, qr) {
  Swal({
    title: title,
    html: '<a href="' + qr + '" download="' + title + '.png"><img src="' + qr + '" class="my-5" style="max-width: 320px"></a>',
    input: 'textarea',
    inputValue: url,
    width: '480px',
