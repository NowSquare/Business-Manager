...
class Localization extends \App\Http\Controllers\Controller {
  ...
  {
    $current_language = (auth()->check()) ? auth()->user()->language : \App::getLanguage();
   ...
   * Get all currencies
   */
  public static function getAllCurrencies($flat_array = true, $data_list_symbol = false)
  {
    ...
    foreach ($currencies as $currency_code => $currency) {
      ...
      //if (in_array($currency_code, $availableCurrencies)) {
        if ($flat_array) {
          ...
          //$return[$currency_code] = $currency->getName() . ' ' . $currency->getSymbol() . ' (' . $currency_code . ')';
        } elseif ($data_list_symbol) {
          $return[] = ['id' => $currency_code, 'data' => ['data-data' => '{"symbol": "' . $currency->getSymbol() . '"}']];
        } else {
